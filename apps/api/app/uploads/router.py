...
async def presign_upload(
    payload: PresignRequest, session: AsyncSession = Depends(get_session)
) -> PresignResponse:
...
async def get_upload(
    image_id: uuid.UUID, session: AsyncSession = Depends(get_session)
) -> ImageUpload:
...
async def list_uploads(
    ...
    stmt = (
        select(ImageUpload)
        .where(ImageUpload.deleted_at.is_(None))
